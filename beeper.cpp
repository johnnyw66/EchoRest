...
size_t playBeep( int __freq, int __timems, int __maxval) {
         size_t writeSize = 0;
        ...
        uint16_t *raw = (uint16_t *)calloc(rawLength, sizeof(uint16_t));
        ...
        for (int i = 0; i < count; i++) {
            i2s_write(SPEAK_I2S_NUMBER, raw, _rate, &bytes_written,
                      portMAX_DELAY);
        }
        if (length != 0) {
            i2s_write(SPEAK_I2S_NUMBER, raw, length, &bytes_written,
                      portMAX_DELAY);
        }
        delete raw;
